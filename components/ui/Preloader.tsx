...
export interface PreloaderProps {
  /** Brand Name Display (Default: "VIDD") */
  brandName?: string;
  /** Subtitle or dynamic rotating word cycle */
  words?: string[];
  /** Subtitle if words array is not provided */
  subtitle?: string;
  /** Logo Image URL */
  logoSrc?: string;
  /** Total loading duration in milliseconds (Default: 1600) */
  duration?: number;
  /** Preloader style variant: 'stairs' | 'percentage' | 'curtain' | 'slide' | 'circle' (Default: "stairs") */
  variant?: 'stairs' | 'percentage' | 'curtain' | 'slide' | 'circle';
  /** Number of vertical stairs slats for stairs variant (Default: 5) */
  stairCount?: number;
  /** Direction from which stairs stagger reveal (Default: 'left') */
  stairsRevealFrom?: 'left' | 'right' | 'center';
  /** Callback fired when preloader loading completes */
  ...
  return (
    ...
    >
        ...
        <div className="preloader-stairs-container">
          {stairsIndices.map((stairDelayIndex, idx) => (
            <div
              ...
              style={{ '--stair-index': stairDelayIndex } as React.CSSProperties}
            />
          ))}
        </div>
      )}
      ...
      {variant !== 'stairs' && variant !== 'curtain' && (
        <div className="preloader-stair-col" style={{ width: '100%', '--stair-index': 0 } as React.CSSProperties} />
      )}
